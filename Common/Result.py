...
class CBIR_Result(Result_Base):
    def __init__(self, *args, **kwargs):
        super(CBIR_Result, self).__init__(*args, **kwargs)
        # acc_kNN=acc_kNN, precision_1=precision_1*100, map
        metrics = ['map', 'acc_kNN', 'precision_1']
        for m in metrics:
            if m not in self.keys():
                self[m] = 0.

        self.score_fn = get_score_CBIR if 'score_fn' not in kwargs.keys() else kwargs['score_fn']
    ...
    def compare_to(self, other):
        ...
        c1 = _compare(self.score(), other.score())
        if c1 == 0:
            return _compare(self.map, -other.map)
        else:
            return c1

    # def map_str(self, frmt='.2f'):
    #     return self._metric_str('mae')
    #
    # def rmse_str(self, frmt='.2f'):
    #     return self._metric_str('rmse')
    ...
    def print(self, pre_text='', color=bcolors.FAIL):
        ...
        if hasattr(self, 'map_R'):
            s += f'MAP@R= {self.map_R:.3f}, '

        s += f'Score={self.score():.3f},  MAP={self.map:.3f},  Precision@1={self.precision_1:.3f}, ' \
             f'kNN Accuracy={self.acc_kNN:.3f}'
...
def get_score_CBIR(res):
    return (2*res.map + res.acc_kNN + res.precision_1)/4
...
def print_res(res, color=bcolors.FAIL, pre_text='', conf_matrix=False, summary_sw=False):
    if type(res) in [Reg_Result, CBIR_Result]:
    ...
    if multi_class:
        if summary_sw:
            ...
            s += (f'macro res -> acc:{res.balanced_acc:.2f}, '
                  f'precision:{res.balanced_precision:.2f}, '
                  f'f_score:{res.balanced_f_score:.2f}')
        else:
...
def main():
    r = CBIR_Result(map=34.781, precision_1=45.65, acc_kNN=58.63)
    r2 = CBIR_Result(map=67.781, precision_1=40.65, acc_kNN=58.63)
    r.print()
    # r = Reg_Result()
    # r.mae = 1.5
    # r2 = Reg_Result()
